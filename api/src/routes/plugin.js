...
// Rotas de compatibilidade (plugin antigo)
router.get('/pending-old', authMiddleware, async (req, res) => {
  try {
    const { username } = req.query;
...
router.post('/deliver-old', authMiddleware, async (req, res) => {
  try {
    const { purchaseId } = req.body;
